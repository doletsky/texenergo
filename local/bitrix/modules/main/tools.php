...
class CUtil
{
	...
	public static function GetPopupSize($resize_id, $arDefaults = array())
	{
	    ...
	    $life_time = 60*60*24;
	    $cache_id = 'GetPopupSize'. $resize_id . serialize($arDefaults);
	    $result = '';
	    if (isset($_REQUEST['clear_cache']) && $_REQUEST['clear_cache']=="Y"){ $obCache->Clean($cache_id); }
...
// 			return false;
		...
		$obCache = new CPHPCache;
		$life_time = 60*60*24;
		$cache_id = 'GetPopupOptions' . $wnd_id;
		$result = '';
		if (isset($_REQUEST['clear_cache']) && $_REQUEST['clear_cache']=="Y"){ $obCache->Clean($cache_id); }
		if($obCache->InitCache($life_time, $cache_id)) {
		    $vars = $obCache->GetVars();
...
// 		}
	    ...
	    $cache_id = 'SetPopupOptions' . $wnd_id . serialize($arOptions);
	    if (isset($_REQUEST['clear_cache']) && $_REQUEST['clear_cache']=="Y"){ $obCache->Clean($cache_id); }	    
	    if($obCache->InitCache($life_time, $cache_id)) {
	    	$vars = $obCache->GetVars();
		    $result = $vars['data'];
	    ...
	    if (empty($result)) {
	    	if($obCache->StartDataCache($life_time,$cache_id, array(), "/")) {
