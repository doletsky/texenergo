        ...
        else
        {
            if($arResult["is_ajax_post"] != "Y" && $arResult["CanUserComment"])
            {
                ...
                <div id="form_comment_" style="display:none;">
					...
					<input type="hidden" name="post_ajax_comment" value="Y">
					
					<?foreach($arParams['PARENT_PARAMS'] as $name => $val):?>
						<?if(substr($name, 0, 1) != '~'):?>
                    ...
                    <div class="blog-comment-fields">
                        ...
                        //if (canVote($arResult['Blog']['ID'], $arResult['Post']['ID'], $arResult['userID'])) {
                        	echo 'Рейтинг <input type="text" name="UF_RATE"/>';
                        //}
                        ?>
                        ...
                        <div id="blg-comment-<?=$comment["ID"]?>">
                            <article class="pReviewSingle">
                                <header>
                                    ...
                                    {
                                        if ($arParams["RATING"][$comment["ID"]]["TOTAL_VOTES"] > 0) {
                                            echo "<b>".$arParams["RATING"][$comment["ID"]]["TOTAL_VOTES"]." ".GetMessage("RAITING_TITLE_".wordForm($arParams["RATING"][$comment["ID"]]["TOTAL_VOTES"]))." отзыв полезным</b>";
                                        }
                                    ...
                                    echo "<div class='pReviewRating cat-rating cat-value-raiting-0'></div>";
                                    ?>
                                </header>
                                <section class="pReviewContent">
                                    ...
                                    <div class="blog-comment-meta">
                                        ...
                                        {
                                            global $APPLICATION;
											$comment["urlToShow"] = str_replace($APPLICATION->GetCurPage(), '', $comment["urlToShow"]);
											$comment["urlToShow"] = $arParams['templateFolder'] . $comment["urlToShow"] . '&IBLOCK_ID=' . $arParams['PARENT_PARAMS']['IBLOCK_ID'] . '&ELEMENT_ID=' . $arParams['PARENT_PARAMS']['ELEMENT_ID'];
											?>
                                            <span class="blog-vert-separator"></span>
                                        ...
                                        if(strlen($comment["urlToHide"])>0)
                                        {
											...
											global $APPLICATION;
											$comment["urlToHide"] = str_replace($APPLICATION->GetCurPage(), '', $comment["urlToHide"]);
											$comment["urlToHide"] = $arParams['templateFolder'] . $comment["urlToHide"] . '&IBLOCK_ID=' . $arParams['PARENT_PARAMS']['IBLOCK_ID'] . '&ELEMENT_ID=' . $arParams['PARENT_PARAMS']['ELEMENT_ID'];
											?>
                                            <span class="blog-vert-separator"></span>
                                            <span class="blog-comment-show">
                                                <?if($arParams["AJAX_POST"] == "Y"):?>
                                        ...
                                        {
                                            global $APPLICATION;
											$comment["urlToDelete"] = str_replace($APPLICATION->GetCurPage(), '', $comment["urlToDelete"]);
											$comment["urlToDelete"] = $arParams['templateFolder'] . $comment["urlToDelete"] . '&IBLOCK_ID=' . $arParams['PARENT_PARAMS']['IBLOCK_ID'] . '&ELEMENT_ID=' . $arParams['PARENT_PARAMS']['ELEMENT_ID'];
											?>
                                            <span class="blog-vert-separator"></span>
            ...
            if($arResult["is_ajax_post"] != "Y")
            {
                ...
                {
                    ?>
                    <div id="form_comment_0" class="j_reviews_form" style="display:none;">
			...
			$arParams['templateFolder'] = $templateFolder.'/ajax.php';
