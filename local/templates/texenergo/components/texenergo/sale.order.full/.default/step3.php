	...
	<td valign="top" width="60%">
		...
		<table class="sale_order_full_table">
			...
			<?
				...
				if (isset($arDelivery['STORE_IDS'])) {
					/*
					 * для самовывоза
					*/
	
					$display = $arDelivery["CHECKED"]=="Y" ? 'display:block;' : 'display:none;';
					echo '<div class="j-stores-' . $arDelivery['ID'] . '" style="' . $display .'">';
					$APPLICATION->IncludeComponent(
							"texenergo:catalog.store.list",
							"",
							Array(
									"PHONE" => "Y",
									"SCHEDULE" => "Y",
				...
				?>
					<tr>
						<td valign="top" width="0%">	
							<input class="j-delivery-type" rel="<?= $arDelivery["ID"] ?>" <?if ($arDelivery["CHECKED"]=="Y") echo " checked";?> type="radio" id="ID_DELIVERY_ID_<?= $arDelivery["ID"] ?>" name="<?=$arDelivery["FIELD_NAME"]?>" value="<?= $arDelivery["ID"] ?>">
						</td>
...
<tr>
	<div id="sof-prof-div">
		<?
			PrintOrderPropertyForm($arResult["PRINT_PROPS_FORM"]["USER_PROPS_Y"], $arParams, $arResult['currentDeliveryId'], $arResult['DELIVERY_LOCATION'], $arResult['PERSON_TYPE'], $arResult['PROFILE_VARS']);
		?>
	</div>
</tr>
<tr>
