...
if ($valueIds) {
    ...
    $arFilter = Array (
        "IBLOCK_ID" => 1, "IBLOCK_LID" => SITE_ID, "ACTIVE" => "Y", "CHECK_PERMISSIONS" => "Y", "PROPERTY_GOODS_FILTER_VALUE" => $valueIds
    );
...
if ($goodsIds) {
    ...
    $arSelect = array (
        ...
        'PROPERTY_SHOW_OLD_PRICE', 'PROPERTY_GOODS_ANALOG_LINKS', 'PROPERTY_GOODS_FILTER_VALUE', 'PROPERTY_GOODS_RATE', 'PROPERTY_BLOG_COMMENTS_CNT', 
    ...
    $res = CIBlockElement::GetList (Array (), $arFilter = Array (
        "IBLOCK_ID" => 1, "IBLOCK_LID" => SITE_ID, "ACTIVE" => "Y", "CHECK_PERMISSIONS" => "Y", "ID" => $goodsIds
    ...
    while ( $ob = $res->GetNextElement () ) {
        ...
        if (isset ($ar_res[ 'PROPERTY_GOODS_ANALOG_LINKS_VALUE' ])) {
            $goodsList[ $ar_res[ 'ID' ] ][ 'PROPERTY_GOODS_ANALOG_LINKS_VALUE_IDS' ][ ] = $ar_res[ 'PROPERTY_GOODS_ANALOG_LINKS_VALUE' ];
            $goodsList[ $ar_res[ 'ID' ] ][ 'PROPERTY_GOODS_ANALOG_LINKS_VALUE_IDS' ] = array_unique ($goodsList[ $ar_res[ 'ID' ] ][ 'PROPERTY_GOODS_ANALOG_LINKS_VALUE_IDS' ]);
        }
    }
}
...
if ($categoryIds) {
    ...
    $res2 = CIBlockSection::GetList (Array (), $arFilter = Array (
        "IBLOCK_ID" => 1, "IBLOCK_LID" => SITE_ID, "ACTIVE" => "Y", "CHECK_PERMISSIONS" => "Y", "ID" => $categoryIds
    ), false, $arSelect2);
...
foreach ($arResult[ "SEARCH" ] as &$arItem) {
    if ($arItem[ "PARAM1" ] == "catalog" && $arItem[ "PARAM2" ] == "1" && strpos ($arItem[ "ITEM_ID" ], "S") === false) {
        ...
        if (isset ($goodsList[ $arItem[ "ITEM_ID" ] ])) {
            ...
            $arItem[ "IBLOCK_SECTION_ID" ] = $goodsList[ $arItem[ "IBLOCK_SECTION_ID" ] ];
            ...
            $arItem[ 'PROPERTY_GOODS_ANALOG_LINKS_VALUE_IDS' ] = isset ($goodsList[ $arItem[ "ITEM_ID" ] ][ 'PROPERTY_GOODS_ANALOG_LINKS_VALUE_IDS' ]) && count ($goodsList[ $arItem[ "ITEM_ID" ] ][ 'PROPERTY_GOODS_ANALOG_LINKS_VALUE_IDS' ]) ? true : false;
            ...
            $arItem[ "ITEM_URL" ] = "/catalog/?SECTION_ID=" . $arItem[ "IBLOCK_SECTION_ID" ] . "&ELEMENT_ID=" . $arItem[ "ITEM_ID" ];
        ...
        if (isset ($categoryList[ (int) str_replace ("S", "", $arItem[ "ITEM_ID" ]) ])) {
            ...
            $arItem[ "ITEM_URL" ] = "/catalog/?SECTION_ID=" . $arItem[ "ITEM_ID" ];
        }
    } elseif ($arItem[ "PARAM1" ] == "shop_articles" && $arItem[ "PARAM2" ] == "9" && strpos ($arItem[ "ITEM_ID" ], "S") === false) {
        $arItem[ "PREVIEW_PICTURE" ] = "";
...
if ($goodsIds) {
    $rsElement = CIBlockElement::GetList (array (
        ...
        "ID", "IBLOCK_ID", "NAME", "PROPERTY_GOODS", 
    		//"GOODS_SERIAL"
    ));
...
foreach ($catIds as $catId => $count) {
    ...
    if ($catId > 0 && ! isset ($firstLevel[ $catId ])) {
        ...
        while ( $arPath = $rsPath->GetNext () ) {
            $data = array (
                'ID' => $arPath[ 'ID' ], 'NAME' => $arPath[ 'NAME' ], 'CATALOG_URL' => "/catalog/?SECTION_ID=" . $arPath[ "ID" ], 'COUNT' => 0
            );
