...
// 				include($_SERVER["DOCUMENT_ROOT"].$templateFolder."/step1.php");
			...
			<?php
				/*
			 	* подгрузка файлов для шага 5 
			 	*/
				$display = $arResult["CurrentStep"] == 5 ? 'display:block' : 'display:none';
				echo '<div style="' . $display . '">';
				$APPLICATION->IncludeComponent("bitrix:main.file.input", "drag_n_drop",
						array(
								"INPUT_NAME"=>"ATTACHED_FILES",
								"MULTIPLE"=>"Y",
								"MODULE_ID"=>"main",
								"MAX_FILE_SIZE"=>"",
								"ALLOW_UPLOAD"=>"A",
	...
	<?if ($arResult["CurrentStep"] > 0 && $arResult["CurrentStep"] <= 7):?>
		...
		<input type="hidden" id="j-change-profile-values" name="CHANGE_PROFILE" value="<?= $arResult["CHANGE_PROFILE"] ?>">
	<?endif?>
	...
	<?if ($arResult["CurrentStep"] > 2 && $arResult["CurrentStep"] <= 6):?>
		...
		<input type="hidden" id="j-store-id" name="STORE_ID" value="<?= $arResult["STORE_ID"]; ?>">
