	...
	<?
		...
		foreach ($arResult["SEARCH"] as $results) {
			...
			if ($isFolder === false) {
				...
				if ($results["PARAM1"] == "catalog") {$catalogCount++;} 
			
			}

			if ($results["PARAM1"] == "shop_articles") {$shopArticlesCount++;}
...
<div class="container">
	<div class="twelve">
		<h1 class="headline">поиск в разделе "спецпредложения" <span class="searchQuery">(<?=$arResult["REQUEST"]["QUERY"]?>)<em class="searchQuantity" ><?=$shopArticlesCount?></em></span> </h1>
		<aside class="share">
			<ul>
		...
		<nav class="sidebar sidebar-search">
			<ul class="sidebarSearch-main">
						...
						<?if ($arResult["firstLevel"]) {?>
							...
							<ul class="sidebarSearch-sub">
								...
								<?foreach ($arResult['firstLevel'] as $category) {?>
									...
									<li><a href="<?=$category["CATALOG_URL"]?>"><?=$category["NAME"]?></a><span class="sidebarQnt"><?=$category["COUNT"]?></span></li>
				...
				<? if ( $shopArticlesCount > 0) {?>
					...
					<li><a href="/search/search-actions.php?q=<?=$arResult["REQUEST"]["QUERY"]?>">спецпредложения</a><span class="sidebarQnt"><?=$shopArticlesCount?></span></li>
...
<?endif;?>
	...
	<input type="hidden" name="section" value="<?echo isset($_REQUEST["section"])? $_REQUEST["section"]: ''; ?>" />
