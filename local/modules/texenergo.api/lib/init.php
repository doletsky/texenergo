...
class Init
{
    ...
    {
	$file_options = $_SERVER['DOCUMENT_ROOT'] . '/bitrix/rest_api_options.php';
    ...
    private function checkToken()
    {
        ...
        if ($_SERVER['HTTP_AUTHORIZATION_TOKEN']) {
                ...
                $tokenQuery = trim($_SERVER['HTTP_AUTHORIZATION_TOKEN']);
                $tokenAPI = trim($this->getParameter()['AUTH_TOKEN']);
		//pr("tokenQuery: ".$tokenQuery);
		//pr("tokenAPI: ".$tokenAPI);

                if ($tokenQuery == $tokenAPI) {
                    $access = true;
		   //pr("Проверка прошла");
		} else {
		//pr("Неверный ключ");
