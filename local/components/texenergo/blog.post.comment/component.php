 ...
 * Рейтинг товара.
 */
function updateRate($elementId, $postId){
			...
			else
			{
				...
				if($arResult["Perm"]>=BLOG_PERMS_MODERATE && !empty($arComment))
				{
					if(check_bitrix_sessid())
					{
						...
						{
						    if (isset($_GET['ELEMENT_ID']) && isset($arComment['POST_ID']) && (int) $_GET['ELEMENT_ID'] && (int)$arComment['POST_ID']) {
						    	$newGoodsRate = updateRate($_GET['ELEMENT_ID'], $arComment['POST_ID']);
						    }
				...
				if($arResult["Perm"] >= BLOG_PERMS_PREMODERATE)
				{
					if(check_bitrix_sessid())
					{
						...
						if($_POST["act"] != "edit")
						{
							...
							if(IntVal($_POST["UF_RATE"])>0 && IntVal($_POST["UF_RATE"]) <= 5 && IntVal($_POST["UF_RATE"]) >=0)
								$arFields["UF_RATE"] = IntVal($_POST["UF_RATE"]);
							else
								$arFields["UF_RATE"] = 0;
							...
							if(strlen($strErrorMessage)<=0)
							{
								...
								if($commmentId = CBlogComment::Add($arFields))
								{
									...
									//SetUserField ("BLOG_COMMENT", $commmentId, "UF_RATE", $arFields['UF_RATE']);
			...
			else
			{
				...
				else
				{
					...
					if(IntVal($arParams["ID"]) > 0)
					{
						...
						$arSelectedFields = Array("ID", "BLOG_ID", "POST_ID", "PARENT_ID", "AUTHOR_ID", "AUTHOR_NAME", "AUTHOR_EMAIL", "AUTHOR_IP", "AUTHOR_IP1", "TITLE", "POST_TEXT", "DATE_CREATE", "PUBLISH_STATUS", "UF_RATE");
