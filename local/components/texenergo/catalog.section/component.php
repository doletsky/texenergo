...
/**
 *
 * Элементам добавлены признаки нахождения в корзине, сравнении, избранном
 * Добавлены поля PRICE и OLD_PRICE для вывода цен в шаблонах
 * Убрана зависимость от SECTION_ID – контекст устанавливается только если указан SECTION_ID,
 * в противном случае для каждого товара урл выбирается отдельно – это позволяет выводить новинки, спецпредложения и лидеров из всего каталога
 *
 * UPD доработан, чтобы была возможность в IBLOCK_IDS передавать массив
 */
...
if ($this->StartResultCache(false, array(array_merge($arrFilter), ($arParams["CACHE_GROUPS"] === "N" ? false : $USER->GetGroups()), $arNavigation))) {
    ...
    $arSelect = array();
    $arParams['SECTION_USER_FIELDS'][] = 'UF_FILTER_ID';
    $arParams['SECTION_USER_FIELDS'][] = 'UF_VALUE_IDS';
    ...
    $goodsIdsInBasket = getGoodsIdsInBasket();
    $goodsIdsInCompareList = getGoodsIdsInCompareList();
    $goodsIdsInFavorites = getGoodsIdsInFavorites();
	...
	if(isset($arParams['IBLOCK_IDS']) && count($arParams['IBLOCK_IDS']) > 0){
		$arFilter['IBLOCK_ID'] = $arParams['IBLOCK_IDS'];
	}
    ...
    if ($arParams["BY_LINK"] !== "Y" && !$arParams["SHOW_ALL_WO_SECTION"] && $arResult['ID'] > 0)
        $rsElements->SetSectionContext($arResult);
    $arResult["ITEMS"] = array();
    $arKeyMap = array();
    $arMeasureMap = array();
    $intKey = 0;
    $resultItemsIdsList = array();
...
//        $arItem["IPROPERTY_VALUES"] = $ipropValues->getValues();
        ...
        $arItem['IN_BASKET'] = in_array($arItem['ID'], $goodsIdsInBasket);
        $arItem['IN_COMPARE_LIST'] = in_array($arItem['ID'], $goodsIdsInCompareList);
        $arItem['IN_FAVORITES'] = in_array($arItem['ID'], $goodsIdsInFavorites);
            ...
            while ($arMeasure = $rsMeasures->GetNext()) {
                ...
                if (isset($arMeasureMap[$arMeasure['ID']]) && !empty($arMeasureMap[$arMeasure['ID']])) {
                    foreach ($arMeasureMap[$arMeasure['ID']] as &$intOneKey) {
                        if (isset($arResult['ITEMS'][$intOneKey])) {
    ...
    $arBasePriceType = CCatalogGroup::GetBaseGroup();
    $arPricesFilter = array(
        'CATALOG_GROUP_ID' => $arBasePriceType['ID'],
        'PRODUCT_ID' => $resultItemsIdsList
    );
    $arPricesSelect = array('ID', 'PRODUCT_ID', 'EXTRA_ID', 'CATALOG_GROUP_ID', 'PRICE', 'CURRENCY', 'TIMESTAMP_X',
    ...
    if(!empty($arResult["ITEMS"])){
        ...
        while ($priceResult = $rsPricesResult->Fetch())
        {
            $priceResult['BASE'] = 'Y';
            $priceResult['CATALOG_GROUP_NAME'] = $arBasePriceType['NAME'];
            $allPricesList[$priceResult['PRODUCT_ID']] = $priceResult;
        }
    }
    ...
    foreach ($arResult["ITEMS"] as &$arItem) {
        ...
        $arItem['PRICE'] = 0;
        $arPrice = $allPricesList[$arItem['ID']];
        if ($arPrice) {
