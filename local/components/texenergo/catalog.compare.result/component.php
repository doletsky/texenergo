 ...
 * Изменен принцип по которому сравниваются товары (в связи со структурой свойств товаров).
 */
...
if (!isset($_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["TEXENERGO_ONLY"]))
    $_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["TEXENERGO_ONLY"] = false;
if (isset($_REQUEST["TEXENERGO_ONLY"]))
    $_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["TEXENERGO_ONLY"] = $_REQUEST["TEXENERGO_ONLY"] == 1;
$arResult["TEXENERGO_ONLY"] = $_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["TEXENERGO_ONLY"];
...
if (is_array($arCompare) && count($arCompare) > 0) {
    ...
    $texenergo = isset($_REQUEST['TEXENERGO_ONLY']) && (int)$_REQUEST['TEXENERGO_ONLY'];
    if ($texenergo) {
        $arFilter['PROPERTY_GOODS_TEXENERGO_ON'] = 1;
    }
    ...
    $arResult['my_compare_property_list'] = array();
    $arResult['goods_property_list'] = array();
...
// 		if ($texenergo) {
// 			$field = $obElement->GetProperty('GOODS_TEXENERGO_ON');
// 			if ($field['VALUE'] != 1) {
// 				continue;
// 			}
// 		}
        $goodsIds[] = $arItem['ID'];
        if (!$categoryId) {
            $categoryId = $arItem['IBLOCK_SECTION_ID'];
        }
    }

    /*
     * TO получить текущие фильтры
     */


    $result = getGoodsProperty($categoryId, $goodsIds);

    if (count($result)) {
        $arResult['goods_property_list'] = $result['goodsList'];
        $arResult['my_compare_property_list'] = $result['filterList'];
    }
...
// 		}
        ...
        if ($arItem["PROPERTIES"]['GOODS_FILTER_VALUE']['VALUE']) {
            $rsProducerElements = CIBlockElement::GetList(
                array("ID" => "DESC"),
                array(
                    'IBLOCK_ID' => 4,
                    'ID' => $arItem["PROPERTIES"]['GOODS_FILTER_VALUE']['VALUE']
                ), array("ID", 'XML_ID'));
            $ids = array();
            while ($obProducerElement = $rsProducerElements->GetNextElement()) {
                $arProducerItem = $obProducerElement->GetFields();
                $ids[] = $arProducerItem['XML_ID'];
            }
            if (!empty($ids)) {
                $rsProducerElements1 = CIBlockElement::GetList(
                    array("ID" => "DESC"),
                    array(
                        'IBLOCK_ID' => 2,
                        'PROPERTY_VALUE_ID' => $ids
                    ));
                if ($obProducerElement1 = $rsProducerElements1->GetNextElement()) {
                    $arProducerItem1 = $obProducerElement1->GetFields();
                    $arProducerItem1["PREVIEW_PICTURE"] = (0 < $arProducerItem1["PREVIEW_PICTURE"] ? CFile::GetFileArray($arProducerItem1["PREVIEW_PICTURE"]) : false);
                    $arItem["PRODUCER"] = $arProducerItem1;
                }
            }
        }
