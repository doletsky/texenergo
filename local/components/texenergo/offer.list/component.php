...
/*
 * Новая компонента.
 * Пункт ТЗ 5. Описание шаблона. Спец. предложения.
 */
...
//			$arItem["IPROPERTY_VALUES"] = $ipropValues->getValues();
            ...
            foreach($arItem['PROPERTIES'] as $properties) {
                if ($properties['CODE'] == 'GOODS' && is_array($properties['VALUE']))
                    $subIds = array_merge($subIds, $properties['VALUE']);
//                 if ($properties['CODE'] == 'GOODS_SERIAL')
//                     $subIds[] = $properties['VALUE'];
            }
        ...
        if (!empty($subIds)) {
            $rsSubElement = CIBlockElement::GetList(array('ID' => 'ASC'), array(
                "IBLOCK_ID" => array(1, 6),
                "ID" => $subIds,
                "IBLOCK_LID" => SITE_ID,
                "ACTIVE" => "Y",
                "CHECK_PERMISSIONS" => "Y",
                "PROPERTY_CATEGORY" => $arResult["ID"]
            ), false, array ('nTopCount' => 100000), array(
                "ID",
                "IBLOCK_ID",
                "IBLOCK_SECTION_ID",
                "NAME",
            ...
            while($obSubElement = $rsSubElement->GetNextElement())
            {
                ...
                $arItem["PROPERTIES"] = $obSubElement->GetProperties();
                $subItems[$arItem['ID']] = $arItem;
            }
            if (CModule::IncludeModule("catalog")) {
        	    foreach($arResult["ITEMS"] as &$item) {
            	    foreach($item['PROPERTIES'] as $properties) {
                	    if ($properties['CODE'] == 'GOODS') {
                	    	$item['GOODS_LIST'] = array();
        					if (is_array($properties['VALUE'])) {
        						foreach ($properties['VALUE'] as $key => $valueId) {
                    	    		$item['GOODS_LIST'][$key] = $subItems[$valueId];//множественное
                    	    		$elem = $item['GOODS_LIST'][$key];
                    	    		$idElem = $elem['ID'];
                    	    		$item['GOODS_LIST'][$key]["COMPARE_URL"] ="/personal/compare.php?action=ADD_TO_COMPARE_LIST&id=".$idElem;
                    	    		$item['GOODS_LIST'][$key]["~COMPARE_URL"] = htmlspecialcharsbx($elem["COMPARE_URL"]);
                    	    		$item['GOODS_LIST'][$key]["COMPARE_DELETE_URL"] ="/personal/compare.php?action=DELETE_FROM_COMPARE_LIST&id=".$idElem;
                   	 	    		$item['GOODS_LIST'][$key]["~COMPARE_DELETE_URL"] = htmlspecialcharsbx($elem["COMPARE_DELETE_URL"]);
                    	    		$item['GOODS_LIST'][$key]["BUY_URL"] = $APPLICATION->GetCurPageParam("action=BUY&id=".$idElem, array('id', 'action'));
                    	    		$item['GOODS_LIST'][$key]["~BUY_URL"] = htmlspecialcharsbx($elem["BUY_URL"]);
                    	    		$item['GOODS_LIST'][$key]['PRICE_PROPERTIES'] = CPrice::GetBasePrice($idElem);
        						}
        					}
        				}
//                 	    if ($properties['CODE'] == 'GOODS_SERIAL')
//                     	    $item['GOODS_SERIAL'] = $subItems[$properties['VALUE']];
            	    }
        	    }
            }
        }
