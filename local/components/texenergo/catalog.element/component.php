...
/*
 * Компонента изменена.
 * Пункт ТЗ 1.14. Страница товара.
 * Добавлены к товару наименования фильтров и наименования значений и др. переменные.
*/
...
if (isset($_COOKIE['catalog_sort']) && (int) $_COOKIE['catalog_sort']) {
	$additionalFilter['catalog_sort'] = $_COOKIE['catalog_sort'];
}
if (isset($_COOKIE['goods_full_view']) && (int) $_COOKIE['goods_full_view']) {
	$additionalFilter['goods_full_view'] = $_COOKIE['goods_full_view'];
}
if (isset($_COOKIE['texenergo_vendor']) && !empty($_COOKIE['texenergo_vendor'])) {
	$additionalFilter['texenergo_vendor'] = $_COOKIE['texenergo_vendor'];
}

/*************************************************************************
			Work with cache
*************************************************************************/
if($this->StartResultCache(false, array($additionalFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))))
{
	...
	if($arParams["ELEMENT_ID"] > 0)
	{
		...
		if($obElement = $rsElement->GetNextElement())
		{
			...
			$arResult["~COMPARE_URL"] ="/personal/compare.php?action=ADD_TO_COMPARE_LIST&id=".$arResult["ID"];
			$arResult["COMPARE_URL"] = htmlspecialcharsbx($arResult["~COMPARE_URL"]);
			$arResult["~COMPARE_DELETE_URL"] ="/personal/compare.php?action=DELETE_FROM_COMPARE_LIST&id=".$arResult["ID"];
			$arResult["COMPARE_DELETE_URL"] = htmlspecialcharsbx($arResult["~COMPARE_DELETE_URL"]);
			$arResult['IN_COMPARE_LIST'] = false;
			if (is_array($goodsIdsInCompareList) && in_array($arResult['ID'], $goodsIdsInCompareList)) {
				$arResult['IN_COMPARE_LIST'] = true;
			}
			...
			)
			{
				...
				foreach($arOffers as $arOffer)
				{
					...
					$arOffer["~COMPARE_DELETE_URL"] ="/personal/compare.php?action=DELETE_FROM_COMPARE_LIST&id=".$arOffer["ID"];
					$arOffer["COMPARE_DELETE_URL"] = htmlspecialcharsbx($arOffer["~COMPARE_DELETE_URL"]);
					if (is_array($goodsIdsInCompareList) && in_array($arResult['ID'], $goodsIdsInCompareList)) {
						$arOffer['IN_COMPARE_LIST'] = true;
					}
			...
			*/
			...
			if (isset($arResult['PROPERTIES']['GOODS_FILTER_VALUE']['VALUE']) && is_array($arResult['PROPERTIES']['GOODS_FILTER_VALUE']['VALUE'])) {
				foreach ($arResult['PROPERTIES']['GOODS_FILTER_VALUE']['VALUE'] as $itemId) {
					$valuesIds[] = $itemId;
				}
			}
			...
			if (count($valuesIds)) {
				...
				if (isset($arResult['PROPERTIES']['GOODS_FILTER_VALUE']['VALUE']) && is_array($arResult['PROPERTIES']['GOODS_FILTER_VALUE']['VALUE'])) {
					foreach ($arResult['PROPERTIES']['GOODS_FILTER_VALUE']['VALUE'] as $id) {
						if (isset($valuesList[$id])) {
							$curentFilterIds[] = $valuesList[$id]['IBLOCK_SECTION_ID'];
						}
					}
				}
				$curretFilterList = getVisibleFilterListByIds($curentFilterIds);

				if (is_array($curretFilterList)) {
					if (isset($arResult['PROPERTIES']['GOODS_FILTER_VALUE']['VALUE']) && is_array($arResult['PROPERTIES']['GOODS_FILTER_VALUE']['VALUE'])) {
						foreach ($arResult['PROPERTIES']['GOODS_FILTER_VALUE']['VALUE'] as $id) {
							if (isset($valuesList[$id])) {
								$filterId = $valuesList[$id]['IBLOCK_SECTION_ID'];
								if (isset($curretFilterList[$filterId])) {
									if ((int)$curretFilterList[$filterId]['UF_SERIAL']) {
										$arResult['arSerial'] = $valuesList[$id];
									}
								    $arResult['PROPERTIES']['GOODS_FILTER_VALUE']['VALUE_NAMES'][$id] = $valuesList[$id];
									$arResult['PROPERTIES']['GOODS_FILTER_VALUE']['VALUE_NAMES'][$id]['FILTER_VARS'] = $curretFilterList[$filterId];
								}
							}
						}
					}
			...
			*/
			if (isset($arResult['PROPERTIES']['GOODS_PHOTOS']['VALUE']) && is_array($arResult['PROPERTIES']['GOODS_PHOTOS']['VALUE'])) {
				foreach ($arResult['PROPERTIES']['GOODS_PHOTOS']['VALUE'] as $k => $fileId) {
					$arResult['PROPERTIES']['GOODS_PHOTOS']['VALUE'][$k] = (0 < $fileId ? CFile::GetFileArray($fileId) : false);
				}
			}
				
			if (isset($arResult['PROPERTIES']['GOODS_SHEME']['VALUE'])) {
				$fileId = (int)$arResult['PROPERTIES']['GOODS_SHEME']['VALUE'];
				$arResult['PROPERTIES']['GOODS_SHEME']['VALUE'] = (0 < $fileId ? CFile::GetFileArray($fileId) : false);
			}
				
			if (isset($arResult['PROPERTIES']['GOODS_DOCS']['VALUE']) && is_array($arResult['PROPERTIES']['GOODS_DOCS']['VALUE'])) {
				foreach ($arResult['PROPERTIES']['GOODS_DOCS']['VALUE'] as $k => $fileId) {
					$arResult['PROPERTIES']['GOODS_DOCS']['VALUE'][$k] = (0 < $fileId ? CFile::GetFileArray($fileId) : false);
				}
			}
			
			$arResult['ADITIONAL_PARAMS'] = CCatalogProduct::GetByID($arResult['ID']);
