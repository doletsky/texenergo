...
if ($groupID > 0) {
    ...
    while ($arID = $rsIDS->Fetch()) {
        $arGroupIDS[] = $arID['LOCATION_ID'];
    }
}


if ($arParams["LOCATION_VALUE"] > 0) {
    if (empty($arGroupIDS) || in_array($arParams["LOCATION_VALUE"], $arGroupIDS)) {
...
$arDefFilter = array("LOC_DEFAULT" => "Y", "LID" => LANGUAGE_ID);

if (!empty($arGroupIDS)) {
    $arDefFilter['ID'] = $arGroupIDS;
}
