...
?>
<form action="/ajax/deliveryForm.php" enctype="multipart/form-data" class="deliveryForm" method="post" data-slide-enable="false"
      data-ajax-form>
        <aside class="popup-order">
            ...
            <header>
                <h1 class="h1-popup">заявка на доставку</h1>
            ...
            <section class="section-popup">
                ...
                <div class="row-popup">
                    <span class="param-popup">КПП</span>
                    <span class="value-popup"><?= $arResult['AGENT']['PROPERTIES']['KPP']['VALUE'] ?></span>
                </div>
                <div class="row-popup">
                    <span class="param-popup">ИНН</span>
                    <span class="value-popup"><?= $arResult['AGENT']['PROPERTIES']['INN']['VALUE'] ?></span>
                </div>
                <div class="row-popup">
                ...
                <div class="row-popup">
                    ...
                    <a href="mailto:<?=$arResult['AGENT']['PROPERTIES']['EMAIL']['VALUE'][0]?>"
                       class="value-popup link-popup"><?=implode(", ", $arResult['AGENT']['PROPERTIES']['EMAIL']['VALUE'])?></a>
                </div>
                <div class="row-popup">
            ...
            <section class="section-popup">
                <h2 class="h2-popup">Доставка</h2>
                <? foreach($arResult['ENUM_LISTS']['DELIVERY'] as $key => $val): ?>
                    <div class="wrap-input">
                        <input id="ship-soon<?=$val['ID']?>" name="fields[delivery]" class="radio-gray" type="radio" value="<?= $val['ID'] ?>" <?= !$key
                            ? 'checked' : '' ?>>
                        <label for="ship-soon<?=$val['ID']?>"><?= $val['VALUE'] ?></label>
                    </div>
                <? endforeach; ?>

                <h3 class="h3-popup">Адрес доставки</h3>
                <textarea class="textarea-popup" name="fields[delivery_address]" id="adress-popup" cols="30"
                          rows="5" required></textarea>

                <div class="wrap-links">
                    <a class="link-popup-big select-address" href="javascript:"><em>+</em> Указать на карте</a>
                    <a class="link-popup-big add-route" href="javascript:"><em>+</em> Прикрепить схему проезда</a>
                    <p class="route-file-name"></p>
                    <input type="file" name="route_file" class="hidden route-file"/>
                </div>
                <div class="select-city-map hidden" id="select-city-map"></div>
                <div class="wrap-form-time">
                    <h3 class="h3-popup l-inline">Часы работы</h3>
                    <span>C</span>
                    <input class="input-popup input-popup-time" data-hours-mask name="fields[work_hours_from]"
                           type="text" required>
                    <span>По</span>
                    <input class="input-popup input-popup-time" data-hours-mask name="fields[work_hours_to]"
                           type="text" required>
                </div>
            </section>
            ...
            <section class="section-popup section-popup-comments">
                ...
                <textarea id="shipping-comments" name="fields[comment]" class="textarea-popup"
