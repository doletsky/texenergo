...
<form action="/ajax/deliveryForm.php" enctype="multipart/form-data" class="deliveryForm" method="post"
    ...
    <aside class="popup-order">
        <div data-fields>
            ...
            <header>
                <h1 class="h1-popup">Доставка третьему лицу</h1>
            ...
            <section class="section-popup group">
                <h2 class="h2-popup">Реквизиты плательщика</h2>
                <? foreach($arResult['ENUM_LISTS']['USER_IS_PAYER'] as $key => $val): ?>
                    <div class="wrap-input">
                        <input id="order-pay<?= $val['ID'] ?>" name="fields[user_is_payer]" class="checkbox-gray"
                               type="checkbox" value="<?= $val['ID'] ?>">
                        <label class="label-grayed" for="order-pay<?= $val['ID'] ?>"><?= $val['VALUE'] ?></label>
                    </div>
                <? endforeach; ?>

                <div class="block-source-full">
                    <h3 class="h3-popup">Наименование организации</h3>
                    <input class="input-popup input-popup-org" name="fields[payer_org]" type="text">
                </div>

                <div class="block-source-half first">
                    <h3 class="h3-popup">ИНН</h3>
                    <input type="text" name="fields[payer_inn]" class="input-popup input-popup-full">
                </div>
                <div class="block-source-half">
                    <h3 class="h3-popup">КПП</h3>
                    <input type="text" name="fields[payer_kpp]" class="input-popup input-popup-full">
                </div>
                <div class="block-source-b">
                    <div class="contact-phone-list">
                        <h3 class="h3-popup">Номер телефона</h3>
                        <input class="input-popup input-popup-phone-long only phone-template"
                               name="fields[payer_phone][]"
                ...
                <div class="row-clear">
                    <h3 class="h3-popup">Оригиналы бухгалтерских документов выслать по адресу:</h3>
                    <? foreach($arResult['ENUM_LISTS']['SEND_DOCUMENTS'] as $key => $val): ?>
                        <div class="wrap-input">
                            <input id="account-adress<?= $val['ID'] ?>" name="fields[send_documents]" class="radio-gray"
                                   type="radio" data-xml-id="<?= $val['XML_ID'] ?>" value="<?= $val['ID'] ?>" <?= !$key ? 'checked' : '' ?>>
                            <label for="account-adress<?= $val['ID'] ?>"><?= $val['VALUE'] ?></label>
                        </div>
                    <? endforeach; ?>
                </div>
                <div class="row-clear hidden send-documents-address">
                    <div class="textarea-50">
                        <h3 class="h3-popup">Адрес отправки бухгалтерских документов</h3>
                        <textarea name="fields[send_documents_address]" id="" class="textarea-popup need-required" cols="30"
                                  rows="5"></textarea>
                    </div>
                </div>
            </section>
            ...
            <section class="section-popup section-popup-comments">
                <h2 class="h2-popup">Транспортная компания</h2>

                <p class="p-gray">Мы доставим груз до дверей выбранной транспортной компании</p>

                <div class="wrap-shipping-select group">
                    <? foreach($arResult['TK_LIST'] as $code => $tk): ?>
                        <div class="block-33">
                            <div class="image-frame">
                                <span class="helper"></span>
                                <? if(strlen($tk['LOGO']) > 0): ?>
                                    <img src="<?= $tk['LOGO'] ?>" alt="<?= $tk['TITLE'] ?>">
                                <? endif; ?>
                            </div>
                            <div class="wrap-input">
                                <input id="logistics-pochta<?= $code ?>" name="fields[tk]" value="<?= $tk['TITLE'] ?>"
                                       class="radio-gray" type="radio" required>
                                <label for="logistics-pochta<?= $code ?>"><?= $tk['TITLE'] ?></label>
                            </div>
                        </div>
                    <? endforeach; ?>
                </div>
