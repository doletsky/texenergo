...
?>
<script src="<?= $componentPath."/js/lib.js" ?>" type="text/javascript"></script>
<script src="<?= $componentPath."/js/vendor/parsley.min.js" ?>" type="text/javascript"></script>
<script src="<?= $componentPath."/js/script.js" ?>" type="text/javascript"></script>
...
<form action="/ajax/deliveryForm.php" enctype="multipart/form-data" class="deliveryForm" method="post"
    ...
    <aside class="popup-order">
        <div data-fields>
            ...
            <header>
                <h1 class="h1-popup">Заявка на предварительный подбор</h1>
            ...
            <section class="section-popup">
                <h2 class="h2-popup">Отправка</h2>

                <? foreach($arResult['ENUM_LISTS']['DELIVERY'] as $key => $val): ?>
                    <div class="wrap-input">
                        <input id="ship-soon<?= $val['ID'] ?>" name="fields[delivery]" class="radio-gray" type="radio"
                               value="<?= $val['ID'] ?>" <?= !$key ? 'checked' : '' ?>>
                        <label for="ship-soon<?= $val['ID'] ?>"><?= $val['VALUE'] ?></label>
                    </div>
                <? endforeach; ?>
            </section>
            <section class="section-popup group">
                ...
                <div class="type_private_person person-type-block active">
                    <div class="block-source-a">
                        ...
                        <input class="input-popup input-popup-name need-required" name="fields[receiver_fio]" type="text" required>
                    </div>
                    <div class="block-source-b ">
                        <div class="contact-phone-list">
                            <h3 class="h3-popup">Номер телефона</h3>
                            <input class="input-popup input-popup-phone-long only phone-template need-required" name="fields[receiver_phone][]"
                ...
                <div class="type_organization person-type-block">
                    <div class="block-source-full">
                        ...
                        <input class="input-popup input-popup-org need-required" name="fields[receiver_org]" type="text">
                    </div>

                    <div class="block-source-third">
                        <h3 class="h3-popup">ИНН</h3>
                        <input type="text" name="fields[receiver_inn]" class="input-popup input-popup-full need-required">
                    </div>
                    <div class="block-source-third">
                        <h3 class="h3-popup">КПП</h3>
                        <input type="text" name="fields[receiver_kpp]" class="input-popup input-popup-full need-required">
                    </div>
                    <div class="block-source-third last">
                        <h3 class="h3-popup">ОКПО</h3>
                        <input type="text" name="fields[receiver_okpo]" class="input-popup input-popup-full need-required">
                    </div>
                    ...
                    <div class="textarea-50">
                        ...
                        <textarea name="fields[delivery_point]" id="" class="textarea-popup need-required" cols="30" rows="5"></textarea>
                    </div>
                    <div class="block-source-b">
                        <div class="contact-phone-list">
                            <h3 class="h3-popup">Номер телефона</h3>
                        ...
                        <a class="link-popup-big l-margin-top-10 add-phone" href="javascript:"><em>+</em> Добавить еще один</a>
                    </div>
                </div>
            </section>
            ...
            <section class="section-popup">
                <h2 class="h2-popup">Получение</h2>

                <p class="p-gray">Забор груза гарантируем на дату:</p>

                <div class="calendar" style="margin-bottom: 20px"></div>
                <input type="hidden" name="fields[pickup_date]" value=""/>

                <div class="wrap-input">
                    <input class="checkbox-gray" id="checkbox-200" type="checkbox" required>
                    <label class="label-grayed label-stronger" for="checkbox-200">Оплату за ответственное хранение после
                        3
                        дня гарантируем</label>
                </div>
                <div class="wrap-input">
                    <input class="checkbox-gray" id="checkbox-201" type="checkbox" required>
                    <label class="label-grayed label-stronger" for="checkbox-201">Наличие доверенности (печати)
                        гарантируем</label>

                    <p class="p-gray">Обращаем ваше внимание, на то что в случае отсутствия доверенности или печати
                        передача
                        продукции производится не будет. Транспортные услуги за доставку в этом случае будут считаться
                        выполненными.</p>
                </div>
                <div class="wrap-input">
                    <input class="checkbox-gray" id="checkbox-202" type="checkbox" required>
                    <label class="label-grayed label-stronger" for="checkbox-202">Погрузочно-разгрузочные работы
                        гарантирую
                        произвести самостоятельно</label>

                    <p class="p-gray">Обращаем ваше внимание, что прием доставленной продукции должен быть произведен в
                        течении 30 мин. За простой машины более 30 мин. по не зависящим от водителя причинам
                        производится оплата в размере 500 руб. в час.</p>
                </div>
