...
if ($USER_ID && $_SERVER["REQUEST_METHOD"] == "POST" && array_key_exists("Update", $_POST)) {
...
if ($bVarsFromForm) {
    ...
    $rsSubscription = $obSubscription->GetList(array(), array("USER_ID" => $USER_ID, "ACTIVE" => "Y", "CONFIRMED" => "Y"));
    $arSubscription = $rsSubscription->Fetch();
    if ($arSubscription) {		
...
foreach ($arRubrics as $arRubric) {
    $arResult["RUBRICS"][$arRubric['CODE']][] = array(
        "ID" => $arRubric["ID"],
        "NAME" => $arRubric["NAME"],
