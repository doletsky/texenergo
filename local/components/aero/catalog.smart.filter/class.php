...
class CBitrixCatalogSmartFilter extends CBitrixComponent
{
	...
	public function getIBlockItems($IBLOCK_ID)
	{
		...
		foreach(CIBlockSectionPropertyLink::GetArray($IBLOCK_ID, $this->SECTION_ID) as $PID => $arLink)
		{
			...
			$arProperty = $this->getPropertyById($PID);
	...
	*/
	...
	function makeFilter($FILTER_NAME)
	{
		...
		if(isset($this->arParams["INNER_FILTER_NAME"]) && !empty($this->arParams["INNER_FILTER_NAME"])){
			$INNER_FILTER_NAME = (string)$this->arParams["INNER_FILTER_NAME"];
			global ${$INNER_FILTER_NAME};
			if(is_array(${$INNER_FILTER_NAME})){
				$arFilter = array_merge($arFilter, ${$INNER_FILTER_NAME});
			}
		}
		/**/
